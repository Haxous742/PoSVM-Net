module systolic_array #(
    ...
    output [N*DATA_WIDTH-1:0] acc_out_ws,
    output [N*DATA_WIDTH-1:0] acc_out_os
);

    wire [DATA_WIDTH-1:0] weight_out [N-1:0][N-1:0];
    wire [DATA_WIDTH-1:0] data_out   [N-1:0][N-1:0];
    wire [DATA_WIDTH-1:0] acc_out    [N-1:0][N-1:0];
    ...
    generate
        for (i = 0; i < N; i = i + 1) begin : output_assign
            assign acc_out_ws[i*DATA_WIDTH +: DATA_WIDTH] = acc_out[N-1][i];
            assign acc_out_os[i*DATA_WIDTH +: DATA_WIDTH] = acc_out[0][i];
        end
    endgenerate

endmodule

module PE_new#(parameter N = 8, es = 2)(
    input wire clk,          
    input wire reset,        
    input wire control,      
...
(*DONT_TOUCH = "Yes"*)
posit_mac #(.N(16), .es(2)) posit_mac_inst (
            .clk(clk),
            .in1({data_in, 8'd0}),  // Upcast 8-bit to 16-bit
            .in2({weight, 8'd0}),   // Upcast 8-bit to 16-bit
            .add_in(acc_in),        // Keep this at whatever your bus width is
            .start(start),
            .out(acc_out_reg),
            .done(done)
   );
